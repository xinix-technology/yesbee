...
module.exports = (function() {
  ...
  Route.prototype = {
    ...
    to(uri, options) {
      ...
      var processor = this.getComponentByUri(uri).createProcessor(uri, options);
    ...
    destroy() {
      _.forEach(this.sources, function(source) {
        this.getComponentByUri(source.uri).removeSource(source);
    ...
    start() {
      return co(function *() {
        ...
        yield _.map(this.sources, function(source) {
          return source.start(processChain);
        });
      }.bind(this));
    },
