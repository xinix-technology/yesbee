...
var Server = function(manager) {
    ...
    this.port = manager.props.port;
    this.host = manager.props.host;
        ...
        c.on('data', function(data) {
            ...
            switch(segments[0]) {
                ...
                case 'quit':
                    ...
                    return;
                case 'ls':
                    var services = that.manager.findServices();
                    c.write('Services:\n');
                ...
                case 'start':
                    try {
                        that.manager.startService(segments[1]);
                    } catch(e) {
                        c.write(e.message + '\n');
                    }
                    break;
                case 'stop':
                    try {
                        that.manager.stopService(segments[1]);
                    } catch(e) {
                        c.write(e.message + '\n');
                    }
                    break;
